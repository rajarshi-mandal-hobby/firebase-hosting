...
import { validateBillsInput } from './schemas/bills';
import { logger } from 'firebase-functions';
import z from 'zod/v4';
import { db } from '.';

exports.saveBills = onCall({ cors: false }, async (req) => {
  validateAuth(req);

  const validationResult = validateBillsInput(req.data);
  // Parse and validate input
  if (!validationResult.success) {
    logger.debug('Validation errors:', z.flattenError(validationResult.error));
    return {
      success: false,
...
//   const billsData = validationResult.data;

  // Save bills data to Firestore
  try {
    await db.runTransaction(async (transaction) => {
      const configRef = db.collection('config').doc('globalSettings');
      const currentDate = await configRef.get();
      logger.debug('Current global settings:', currentDate.data());
    });
  } catch (error) {
...
//             totalCharges,
//             amountPaid: 0,
//             currentOutstanding: 1 + totalCharges,
//             note: '',
