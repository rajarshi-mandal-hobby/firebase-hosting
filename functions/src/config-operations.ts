 ...
 */
export const getAdminConfig = onCall({ cors: true }, async (request): Promise<CloudFunctionResponse<AdminConfig>> => {
  try {
    const uid = validateAuth(request);
...
export const getAdminMemberDetails = onCall(
  ...
  > => {
    try {
      ...
      const yearPayments = recentRentHistory.filter((rent) => rent.generatedAt.toDate() >= yearStartDate);

      const totalPaidThisYear = yearPayments.reduce((sum, rent) => sum + rent.amountPaid, 0);
      const averageMonthlyPayment = yearPayments.length > 0 ? totalPaidThisYear / yearPayments.length : 0;
