...
const BillsSection: React.FC<BillsSectionProps> = ({ students, config, loading }) => {
  ...
  const filteredStudents =
    selectedFloor === "all" ? activeStudents : activeStudents.filter((s) => s.floor === selectedFloor);

  const totalOutstanding = filteredStudents.reduce((sum, student) => {
    return sum + (student.currentRent || 0) + (student.currentOutstandingBalance || 0);
  }, 0);
      ...
      <Paper shadow='xs' p='sm' radius='md'>
        <Select
          ...
          data={[
            ...
            ...(config?.floors || []).map((floor) => ({ value: floor, label: floor })),
          ]}
          size='sm'
        />
      </Paper>
      ...
      <Stack gap='xs'>
        {filteredStudents.length === 0 ? (
          ...
          filteredStudents.map((student, index) => {
            ...
            const studentTotal =
              (student.currentRent || 0) + (student.electricityAmount || 0) + (student.wifiAmount || 0);
            ...
            return (
              <Card key={studentId} shadow='xs' radius='md' withBorder>
                <Group
                  ...
                  <Group>
                    <Text size='sm' fw={600}>
                      <NumberFormatter value={studentTotal} prefix='₹' thousandSeparator />
                    </Text>
                ...
                <Collapse in={!isCollapsed}>
                  ...
                  <Stack gap='xs'>
                    <Group justify='space-between'>
                      ...
                      <Text size='sm'>
                        <NumberFormatter value={student.currentRent || 0} prefix='₹' thousandSeparator />
                      </Text>
                    </Group>
                    <Group justify='space-between'>
                      <Text size='sm'>Electricity</Text>
                      <Text size='sm'>
                        <NumberFormatter value={student.electricityAmount || 0} prefix='₹' thousandSeparator />
                      </Text>
                    </Group>
                    <Group justify='space-between'>
                      <Text size='sm'>WiFi</Text>
                      <Text size='sm'>
                        <NumberFormatter value={student.wifiAmount || 0} prefix='₹' thousandSeparator />
                      </Text>
                    </Group>
                    <Divider />
                    <Group justify='space-between'>
