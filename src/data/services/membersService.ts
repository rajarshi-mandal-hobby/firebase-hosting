...
export const fetchMembers = async ({ reload, ...filters }: MemberFilters): Promise<Member[]> => {
  ...
  console.log(`Fetching members with filters: ${cacheKey}, reload: ${reload}`);
  ...
  const newPromise = (async () => {
    ...
    //     throw new Error('Data from cache, fetch from server');
    //   }

      console.log('Members fetched:', metadata.fromCache ? 'from cache' : 'from server');

      const data = snapshot.docs.map((doc) => doc.data());
      console.log(`Fetched ${data.length} members for filters: ${cacheKey}`);
