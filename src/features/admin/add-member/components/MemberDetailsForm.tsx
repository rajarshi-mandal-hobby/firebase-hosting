...
import type { DefaultValues, Floor, BedType, Member, Action } from "../../../../data/types";
...
export default function MemberDetailsForm({ defaultValues, member, action }: MemberDetailsFormProps) {
	...
	const {
		...
		summary,
		actions
	} = useMemberDetailsForm({ defaultValues, member, currentSettingsRent, action });
			...
			{member && shouldDisplayMismatchAlert && (
				<Alert title='Rent Mismatch Detected' color='orange' mb='md'>
					<List size='sm' spacing='xs'>
						...
						{isSecurityDepositMismatch && (
							<List.Item fw={500}>
								Settings: {toIndianLocale(defaultValues.securityDeposit)} &mdash; Member:{" "}
				...
				<Stack>
					...
					<SimpleGrid cols={2} verticalSpacing='sm'>
						...
						<NumberInputWithCurrency
							...
							placeholder={defaultValues.securityDeposit.toString()}
							rightSection={
							...
							rightSectionProps={{
								onClick: () =>
									...
									:	form.setFieldValue("securityDeposit", defaultValues.securityDeposit),
								style: { cursor: "pointer" }
							}}
							rightSectionWidth={34}
							required
					...
					<SimpleGrid cols={2} mt='md'>
						<NumberInputWithCurrency
							...
							rightSection={
								member ?
									<IconEditOff size={ICON_SIZE} />
								:	form.isDirty("amountPaid") && <IconClose size={ICON_SIZE} />
							}
							rightSectionProps={{
					...
					<Textarea
						label={
							<GroupIcon>
								<IconNote size={ICON_SIZE} /> <Text>Notes (Optional)</Text>
							</GroupIcon>
						}
