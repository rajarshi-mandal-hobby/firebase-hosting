...
import { toIndianLocale, formatPhoneNumber, getSafeDate } from '../../../../shared/utils';
...
import { type DefaultRents, type Member, type MemberAction, Floors } from '../../../../data/types';
...
export default function MemberDetailsForm({ defaultRents, member, memberAction: action }: MemberDetailsFormProps) {
    const currentSettingsRent = member ? defaultRents.bedRents[member.floor][member.bedType] : 0;
    ...
    const {
        ...
        actions
    } = useMemberDetailsForm({ defaultRents, member, currentSettingsRent, memberAction: action });
            ...
            {member && shouldDisplayMismatchAlert && (
                <Alert title='Rent Mismatch Detected' color='orange' mb='md'>
                    <List size='sm' spacing='xs'>
                        {isRentMismatch && (
                            <List.Item fw={500}>
                                Settings: {toIndianLocale(currentSettingsRent)} &mdash; Member:{' '}
                ...
                <Stack>
                    ...
                    <TextInput
                        ...
                        rightSection={
                            form.isDirty('name') ?
                                member ?
                                    <IconUndo size={DEFAULT_SVG_SIZE} />
                                :   <IconClose size={DEFAULT_SVG_SIZE} />
                            :   null
                        }
                        rightSectionProps={{
                            onClick: () =>
                    ...
                    <Group grow preventGrowOverflow={false} wrap='nowrap'>
                        <MonthPickerInput
                            ...
                            leftSection={<IconCalendarMonth size={DEFAULT_SVG_SIZE} />}
                        ...
                        <TextInput
                            ...
                            rightSectionWidth={34}
                            leftSection={<IconCall size={DEFAULT_SVG_SIZE} />}
                            required
                            flex={2}
                            key={form.key('phone')}
                    ...
                    <SimpleGrid cols={2} verticalSpacing='sm'>
                        <NumberInputWithCurrency
                            ...
                            required
                            readOnly
                            rightSection={<IconEditOff size={DEFAULT_SVG_SIZE} />}
                            rightSectionWidth={34}
                        ...
                        {!!member && (
                            <NumberInputWithCurrency
                                ...
                                rightSection={
                                    form.isDirty('rentAtJoining') ? <IconUndo size={DEFAULT_SVG_SIZE} /> : null
                                }
                                rightSectionProps={{
                    ...
                    <SimpleGrid cols={2} spacing='xs' verticalSpacing='xs'>
                        ...
                        <Text fw={500}>{toIndianLocale(summary.rentAmount)}</Text>
                        <Group gap='xs'>
                            <IconUniversalCurrency />
                            <Text>Advance Deposit:</Text>
                        </Group>
                        <Text fw={500}>{toIndianLocale(summary.advanceDeposit)}</Text>
                        <Group gap='xs'>
                            <IconUniversalCurrency />
                            <Text>Security Deposit:</Text>
                        </Group>
                        <Text fw={500}>{toIndianLocale(summary.securityDeposit)}</Text>
                        <Group gap='xs'>
                            <IconPayments />
                            <Text fw={700}>Total:</Text>
                        </Group>
                        <Text fw={700}>{toIndianLocale(summary.total)}</Text>
                    </SimpleGrid>

                    {!!member && (
                        <Alert color='indigo' mt='md' icon={<IconMoneyBag />}>
                            Previously Agreed Deposit: <strong>{toIndianLocale(member.totalAgreedDeposit)}</strong>
                        </Alert>
                    )}
                    <SimpleGrid cols={2} mt='md'>
                        <NumberInputWithCurrency
                            ...
                            rightSection={
                                member ?
                                    <IconEditOff size={DEFAULT_SVG_SIZE} />
                                :   form.isDirty('amountPaid') && <IconClose size={DEFAULT_SVG_SIZE} />
                            }
                            rightSectionProps={{
                    ...
                    <Checkbox
                        label={`Forward outstanding amount of ${toIndianLocale(summary.outstanding)}?`}
                    ...
                    <Textarea
                        label={
                            <GroupIcon>
                                <IconNote size={DEFAULT_SVG_SIZE} /> <Text>Notes (Optional)</Text>
                            </GroupIcon>
                        }
                        ...
                        rightSectionProps={{
                            onClick: () =>
                                member ? form.setFieldValue('note', member.note) : form.setFieldValue('note', ''),
                            style: { cursor: 'pointer' }
                        }}
                        maxRows={3}
                        resize='block'
