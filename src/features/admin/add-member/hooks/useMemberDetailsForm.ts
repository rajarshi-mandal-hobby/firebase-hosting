...
import { normalizePhoneInput, formatPhoneNumber, toNumber, formatNumberIndianLocale } from "../../../../shared/utils";
...
export const useMemberDetailsForm = ({
   ...
   const generateNote = (values: MemberDetailsFormData) => {
      ...
      let note = dateText;
      note += action === "edit" ? "Updated" : action === "reactivate" ? "Reactivated" : "Added";
      ...
      if (action === "add") {
         const defaultNote = `\n- At the time of joining, ${summary.total.toIndianLocale()} has been charged, that includes ${formatNumberIndianLocale(summary.rentAmount)} as Rent for the current month, ${formatNumberIndianLocale(summary.advanceDeposit)} as Advance Deposit and ${formatNumberIndianLocale(summary.securityDeposit)} as Security Deposit.`;
         // Get rent note
         const rentNote = values.shouldForwardOutstanding
            ? `${defaultNote}\n- However, ${formatNumberIndianLocale(values.amountPaid)} has been paid creating an Outstanding of ${formatNumberIndianLocale(values.outstandingAmount)}. This amount will be added to the current month's bill and will be forwarded to the next month's bill.`
            : defaultNote;
         note += rentNote;
      }
