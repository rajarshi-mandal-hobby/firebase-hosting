...
import { useTransition, startTransition, useActionState } from 'react';
...
export const useGenerateBillsForm = (billingData: GenerateBillsData): UseGenerateBillsFormReturn => {
  ...
  const [submittedFormData, setSubmittedFormData] = useState<GenerateBillFormData | null>(null);
  ...
  const computeDerivedState = (formValues: GenerateBillFormData): DerivedUIState => {
    return {
      ...
      toggleState: (['2nd', '3rd'] as Floor[]).reduce((acc, floor) => {
        acc[floor] = billingData.activeMembersIdsByFloor[floor].every((id) =>
  ...
  const calculateDeletedMembers = (prevMembers: Record<string, string>) => {
    return Object.keys(prevMembers)
      .filter((memberId) => !billingData.membersOptions.some((opt) => opt.value === memberId))
      .map((memberId) => ({ value: memberId, label: prevMembers[memberId] }));
  };

  const segmentedControlData = Object.values(billingData.billingMonths).map((month) => ({
  ...
  const form = useForm<GenerateBillFormData>({
    mode: 'uncontrolled',
    initialValues: {
      selectedBillingMonth: dayjs(billingData.billingMonths.nextBillingMonth.toDate()).format('YYYY-MM-DD'),
      activeMemberCounts: billingData.activeMembersCounts,
      wifiCharges: billingData.wifiCharges,
      additionalExpenses: {
        addExpenseMemberIds: [],
  ...
  function handleDateChange(values: GenerateBillFormData, previousValues: GenerateBillFormData) {
    ...
    const currentMonthKey = dayjs(billingData.billingMonths.currentBillingMonth.toDate()).format('YYYY-MM-DD');
    const nextMonthKey = dayjs(billingData.billingMonths.nextBillingMonth.toDate()).format('YYYY-MM-DD');
  ...
  const toggleFloorExpense = (floor: Floor, checked: boolean) => {
    ...
    const floorMembers = billingData.activeMembersIdsByFloor[floor];
  ...
  const handleFormSubmit = (values: GenerateBillFormData) => {
    setSubmittedFormData({
      ...
      submittedMembers: {
        ...
        activeMembersIdsByFloor: billingData.activeMembersIdsByFloor,
      },
    });
    openConfirmModal();
  };
  ...
  const handleConfirm = () => {
    if (submittedFormData) {
      console.log('Confirmed form data:', submittedFormData);
      closeConfirmModal();
      startFetching(async () => {
        ...
        if (!result.success) {
          ...
          if (result.errors.formErrors.length > 0) {
            notifyError(result.errors.formErrors.join(', '));
          }
        }
      });
    }
  ...
  const isNextMonth =
    form.values.selectedBillingMonth ===
    dayjs(billingData.billingMonths.nextBillingMonth.toDate()).format('YYYY-MM-DD');
  const memberOptions = isNextMonth ? billingData.membersOptions : [...billingData.membersOptions, ...deletedMembers];
  ...
  return {
    ...
    toggleFloorExpense,
    handleFormSubmit,
    modalActions: { confirmModalOpened, submittedFormData, openConfirmModal, closeConfirmModal, handleConfirm },
  };
};
