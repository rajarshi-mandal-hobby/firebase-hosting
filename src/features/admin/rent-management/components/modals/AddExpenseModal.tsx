...
import { useEffectEvent, useEffect, useCallback, useMemo, useState, useRef } from "react";
import type { Expense } from "../../../../../data/types";
import { MyLoadingOverlay, NumberInputWithCurrency } from "../../../../../shared/components";
import { IconUndo, IconAdd } from "../../../../../shared/icons";
import { notifyLoading, notifyUpdate, formatNumberWithOrdinal, formatNumberIndianLocale } from "../../../../../shared/utils";
...
const expenseFormCache = new Map<string, FormExpenses["expenses"]>();
...
export function AddExpenseModal({
   ...
   const effectEvent = useEffectEvent(() => {
      ...
      // Check cache first, then fall back to initialExpenses or empty state
      const cachedExpenses = expenseFormCache.get(memberName);
      const expenses =
         cachedExpenses || (initialExpenses.length > 0 ? initialExpenses : [{ description: "", amount: "" }]);

      // We must clone to ensure form doesn't mutate our cache reference directly
      form.setValues({ expenses: [...expenses] });
      form.resetDirty();
      hasErrorRef.current = !!cachedExpenses; // If loaded from cache, assume potentially error-prone previous state

      console.log("Effect event triggered", { memberName, cachedExpenses: !!cachedExpenses });
   });
   ...
   // Check if any *new* non-empty expenses have been added (when starting from empty)
   // or if we have more expenses than we started with
   const isAdded = useMemo(() => {
      if (initialExpenses.length === 0) {
         // If we started with nothing, are there any non-empty rows?
         return currentExpenses.some((e) => e.description.trim() !== "" || (e.amount !== "" && Number(e.amount) !== 0));
      }
      // If we started with some, did we add more?
      // Note: This simple length check might be enough, or you might want to check content
      return currentExpenses.length > initialExpenses.length;
   }, [currentExpenses, initialExpenses]);

   const isModified = useMemo((): boolean => {
      for (let i = 0; i < currentExpenses.length; i++) {
         // Only check modification for existing indices
         if (i < initialExpenses.length) {
      ...
      return false;
   }, [currentExpenses, initialExpenses, isExpenseModified]);

   const isRemovedOrModified = isRemoved || isModified || isAdded;

   // Reset error state if form becomes clean
   useEffect(() => {
      if (!isRemovedOrModified) {
         hasErrorRef.current = false;
         expenseFormCache.delete(memberName);
      }
   }, [isRemovedOrModified, memberName]);
   ...
   const handleAddExpenses = async () => {
      ...
      try {
         ...
         // throw new Error('Failed to add expenses'); // Uncomment to test error state
         ...
         expenseFormCache.set(memberName, [...currentExpenses]);
   ...
   const handleExitTransitionEnd = () => {
      // Only clear cache if we didn't have an error (meaning user cancelled or success was already handled)
      // Note: Success case already deletes cache, so cache check handles "Cancel" case.
      // If hasErrorRef is true, we preserve the cache.
      // If hasErrorRef is false, and we are exiting, it means purely Cancel with no active error state.
      if (!hasErrorRef.current) {
         expenseFormCache.delete(memberName);
      } else {
         console.log("Preserving form cache due to error state");
      }
                     ...
                     <Group align='flex-start' justify='space-between'>
                        ...
                        <Group mt={28} wrap='nowrap'>
                           ...
                           <ActionIcon
                              ...
                              disabled={
                                 currentExpenses.length === 1 && expense.description === "" && expense.amount === ""
                              }
                              size={30}>
                              <CloseIcon size='16' />
                           </ActionIcon>
