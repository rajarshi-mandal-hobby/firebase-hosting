...
import { SharedAvatar, RentDetailsList, StatusIndicator, StatusBadge, ErrorBoundary } from '../../../shared/components';
...
export function RentManagement() {
  ...
  const [retryErrorKey, setRetryErrorKey] = useState(0);

  return (
    <>
      {isLoading && <LoaderSleeping />}
      {!!error && <ErrorContainer error={error} onRetry={actions.handleRefetch} />}
      {!isLoading && !error && (
        <Stack>
          <Group gap='xs'>
            <Title order={5}>Total Outstanding:</Title>
            <Title order={5}>{formatNumberIndianLocale(totalOutstanding)}</Title>
