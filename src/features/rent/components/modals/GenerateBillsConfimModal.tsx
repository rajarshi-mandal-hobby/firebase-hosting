import { Stack, Title, Group, Badge, Accordion, List, Button, Text } from '@mantine/core';
...
import type { ModalType } from './GenerateBillsModal';

export function BillingSummary({
  formData,
  actions,
}: {
  formData: GenerateBillFormData;

  actions: {
    handleModalToggle: (modal: ModalType) => void;
    handleModalClose: () => void;
    handleConfirmGenerate: () => void;
  };
}) {
  if (!formData || !formData.submittedMembers) {
    throw new Error('Form data is missing');
  }
  const { handleModalToggle, handleModalClose, handleConfirmGenerate } = actions;

  const { secondFloorExpenseMembers, thirdFloorExpenseMembers, otherExpenseMembers, memberOptionsLookup } =
    useCategorizeMembersByExpense(formData);

  console.log('Rendering billing summary');
    ...
    <Stack gap='xl'>
        ...
        {formData.wifiCharges.wifiMemberIds.length > 0 && !!formData.wifiCharges.wifiMonthlyCharge && (
          <Accordion.Item value='wifi-charges'>
            <Accordion.Control>
              <Title order={6}>
                WiFi Charges: ₹
                {computePerHeadBill(formData.wifiCharges.wifiMonthlyCharge, formData.wifiCharges.wifiMemberIds.length)}
                /Member
              </Title>
            </Accordion.Control>
            <Accordion.Panel>
              ...
              <List size='sm'>
                {formData.wifiCharges.wifiMemberIds.map((id) => (
                  <List.Item key={id}>{memberOptionsLookup[id] || 'Deleted Member'}</List.Item>
                ))}
              </List>
            </Accordion.Panel>
          </Accordion.Item>
      ...
      <Group mt='md' align='center' justify='space-between'>
        ...
        <Group justify='flex-end'>
          <Button variant='transparent' onClick={() => handleModalToggle('confirm')}>
            Edit
          </Button>
          <Button onClick={handleConfirmGenerate}>{formData.isUpdatingBills ? 'Update' : 'Generate'}</Button>
        </Group>
      </Group>
    </Stack>
  );
