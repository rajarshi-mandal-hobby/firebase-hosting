...
import type { Member } from "../components/admin/members/types/member";
 ...
 */
export const calculateStudentsSummary = (students: Member[]) => {
  return {
    totalStudents: students.length,
    activeStudents: students.filter((s) => s.isActive).length,
    totalOutstanding: students.reduce(
      (sum, s) => sum + s.outstandingBalance,
      0,
    ),
    totalDepositsCollected: students.reduce(
      (sum, s) => sum + (s.totalAgreedDeposit - s.outstandingBalance),
      0,
    ),
    wifiOptedCount: students.filter((s) => s.optedForWifi && s.isActive).length,
    floorDistribution: students
      .filter((s) => s.isActive)
      .reduce(
        (acc, s) => {
