...
export const functions = getFunctions(
  app,
  import.meta.env["VITE_FIREBASE_REGION"] || "us-central1",
);
...
if (import.meta.env.DEV && import.meta.env["VITE_USE_EMULATORS"] === "true") {
  ...
  if (!emulatorsConnected) {
    try {
      ...
      const authHost =
        import.meta.env["VITE_EMULATOR_AUTH_HOST"] || "localhost";
    ...
    try {
      ...
      const storageHost =
        import.meta.env["VITE_EMULATOR_STORAGE_HOST"] || "localhost";
      const storagePort = parseInt(
        import.meta.env["VITE_EMULATOR_STORAGE_PORT"] || "9199",
      );
    ...
    try {
      ...
      const functionsHost =
        import.meta.env["VITE_EMULATOR_FUNCTIONS_HOST"] || "localhost";
      const functionsPort = parseInt(
        import.meta.env["VITE_EMULATOR_FUNCTIONS_PORT"] || "5001",
      );
    ...
    console.log(
      "✅ Firestore emulator connected via initializeFirestore settings",
    );
  }
}
...
export const isAdminUser = async (email: string | null): Promise<boolean> => {
  ...
  try {
    ...
    const { getAdminConfig } = await import("./config");
    ...
    console.warn(
      "Could not fetch admin config, falling back to hardcoded list:",
      error,
    );
  }
