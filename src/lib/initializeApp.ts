...
export const initializeApp = async (): Promise<void> => {
  try {
    ...
    const initializeConfigFunction = httpsCallable(functions, 'initializeConfiguration');
    const result = await initializeConfigFunction({});
    
    if (result.data && typeof result.data === 'object' && 'success' in result.data) {
      const response = result.data as { success: boolean; message: string; isNew?: boolean };
      if (response.success) {
        ...
        console.warn(`⚠️ Configuration initialization failed: ${response.message}`);
      }
    }
    ...
    console.log('ℹ️ App will continue - configuration can be initialized from admin panel');
  }
};
