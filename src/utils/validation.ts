 ...
 */
export const validateNumber = (value: unknown, fieldName: string, min = 0): { isValid: boolean; error?: string; value?: number } => {
...
export const validateFloorBedTypes = (
  ...
  for (const bedType of bedTypes) {
    if (floorData[bedType] !== undefined && floorData[bedType] !== null) {
      const validation = validateNumber(floorData[bedType], `${floorName} ${bedType}`);
  ...
  if (!hasValidBedType) {
    errors[`${floorName}.general`] = `At least one bed type must be configured for ${floorName} floor`;
  }
...
export const validateConfigForm = (formData: {
  ...
  for (const floor of floors) {
    if (formData.bedTypes[floor]) {
      const floorValidation = validateFloorBedTypes(formData.bedTypes[floor], floor);
      if (!floorValidation.isValid) {
        Object.assign(errors, floorValidation.errors);
      }
    } else {
      errors[`${floor}.general`] = `Configuration for ${floor} floor is required`;
    }
  }
...
export const sanitizeConfigFormData = (formData: Record<string, unknown>) => {
  return {
    bedTypes: (formData['bedTypes'] as Record<string, Record<string, number>>) || {},
